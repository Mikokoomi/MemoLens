...
Phase 16C tạo nền tảng automated test nhỏ, độc lập với LocalDB phát triển. Mục tiêu là giúp các phase sau kiểm thử auth, quyền sở hữu, database và API mà không làm ảnh hưởng dữ liệu cá nhân trong môi trường Development.
...
Các test này chỉ kiểm tra app có boot được, MVC/API route cơ bản và boundary auth tối thiểu. Chúng không dùng LocalDB, không gửi email và không tạo dữ liệu trong database Development.

## 5. Chưa được cover

- Register/login/confirm email đầy đủ.
- Forgot/reset password MVC và API.
- Refresh token rotation, reuse, logout và cleanup.
...
1. **Phase 16D: Auth API Integration Tests**
   - Register, login, email confirmation, refresh rotation, logout, reset password và safe error response.
2. **Phase 16E: Privacy/Ownership Tests**
   - User A/B isolation cho memory, album, trash, settings và API tương lai.
3. **Phase 16F: Image Access Tests**
   - Authorized image serving, soft delete, missing file và private storage boundary.
4. **Phase 16G: Database Cleanup Tests**
   - Refresh token retention, unused tag và orphan image dry-run/quarantine khi các service đó được triển khai.
5. **Sau đó: UI/end-to-end tests**
