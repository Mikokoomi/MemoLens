...
- Cleanup service dùng UTC và xóa theo batch chỉ khi token đã revoke quá `RevokedTokenRetentionDays` hoặc hết hạn quá `ExpiredTokenRetentionDays`.
- Hosted service được đăng ký nhưng chỉ chạy khi `RefreshTokenCleanup:Enabled` là `true`; cấu hình mặc định là `false`.
- Log cleanup chỉ chứa số record và thời điểm UTC, không chứa plain token hoặc token hash.

Index `UserRefreshTokens.RevokedAt` đã được thêm ở Phase 16B.1. Khi retention/query thực tế lớn hơn, cần đo lại trước khi cân nhắc composite hoặc filtered index.
...
| Thấp | Chưa có cleanup ảnh/tag, quota, thumbnail/compression hoặc retention policy cho trash. | Thiết kế ở Phase 16B.3 và production plan. |
...
1. **Phase 16B.3: Orphan Image and Unused Tag Cleanup Design**
   - Chốt tiêu chí, retention và chiến lược an toàn trước khi xóa file/tag không còn tham chiếu.
