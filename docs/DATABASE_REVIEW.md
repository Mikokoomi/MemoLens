...
| --- | --- |
| `Memories` | `UserId`, `MemoryDate`, `Feeling` (single-column). |
| `Albums` | `UserId`, `UpdatedAt` (single-column). |
| `MemoryImages` | `MemoryId`. |
| `MemoryTags` | PK `(MemoryId, TagId)` và index `TagId`. |
| `AlbumMemories` | PK `(AlbumId, MemoryId)` và index `MemoryId`. |
| `Tags` | `Name` unique. |
| `UserRefreshTokens` | `UserId`, `TokenHash` unique, `ExpiresAt`. |

Đề xuất cho Phase 16B, sau khi đo query plan trên dữ liệu thực tế:

| Mục tiêu query | Index đề xuất |
| --- | --- |
| Timeline của user chưa xóa | `Memories(UserId, IsDeleted, MemoryDate, CreatedAt)`. |
| Album của user chưa xóa | `Albums(UserId, IsDeleted, UpdatedAt, CreatedAt)`. |
| Ảnh bìa/ảnh memory có thứ tự | `MemoryImages(MemoryId, UploadedAt)`. |
| Cleanup refresh token | Cân nhắc `UserRefreshTokens(RevokedAt, ExpiresAt)` hoặc filtered index cho token active, tùy query cleanup thực tế. |
...
1. **Phase 16B: Database Index and Cleanup Implementation**
   - Đo query plan, thêm index đã được chứng minh cần thiết, cleanup refresh token/tag/orphan file và chính sách retention tối thiểu.
